...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Normalize keypoints and save processed versions.")
    parser.add_argument("--input_folder", type=str, required=True, help="Path to input keypoints .npy files folder")
    parser.add_argument("--output_folder_kps", type=str, default=OUTPUT_FOLDER_kps, help="Path to output normalized keypoints .npy files folder")
    parser.add_argument("--output_folder_csv_original", type=str, default=OUTPUT_FOLDER_csv_original, help="Path to output original keypoints CSV folder")
    parser.add_argument("--output_folder_csv_processed", type=str, default=OUTPUT_FOLDER_csv_processed, help="Path to output processed keypoints CSV folder")
    args = parser.parse_args()
    
    input_folder = args.input_folder
    os.makedirs(args.output_folder_kps, exist_ok=True)
    os.makedirs(args.output_folder_csv_original, exist_ok=True) # comment it out if not needed
    os.makedirs(args.output_folder_csv_processed, exist_ok=True) # comment it out if not needed
    ...
    for kp_file in glob.glob(f"{input_folder}/*.npy"):
        try:
            ...
            np.save(os.path.join(args.output_folder_kps, f"{kp_name}.npy"), processed)

            # Save CSV for human-readable inspection (processed), comment out if not needed
            # df = pd.DataFrame(processed.reshape(processed.shape[0], -1))
            # df.to_csv(os.path.join(args.output_folder_csv_processed, f"{kp_name}_processed.csv"), index=False)

            # Save ORIGINAL in CSV, comment out if not needed
            # if orig_resh is not None:
            #     df_orig = pd.DataFrame(orig_resh)
            #     df_orig.to_csv(os.path.join(args.output_folder_csv_original, f"{kp_name}_original.csv"), index=False)
